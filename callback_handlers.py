...
from services.chat_engine import get_simple_user_id, find_model
...
async def chek_ai(callback: CallbackQuery):
    ...
    try:
        ...
        if answer:
            ...
            await callback.message.edit_text(f"ИИ модель изменена на {ai_name}", reply_markup=None)
        else:
...
@router.callback_query(F.data.startswith("deletechannel:"))
async def delete_channel(callback: CallbackQuery):
    ...
    for item in dictionary_channel_id:
        if list(item.values())[0] != int(channel_id_to_delete):
...
@router.callback_query(F.data.startswith("channeldays:"))
async def view_channel_days(callback: CallbackQuery):
    user_id = get_simple_user_id(callback=callback)
    if not user_id:
        return
    ...
    channel_info = next((item for item in dictionary_channel_id if list(item.values())[0] == int(channel_id_to_view)), None)
    ...
    channel_id = list(channel_info.values())[0]
    ...
    try:
        ...
        await callback.message.edit_text(ai_answer)
    except Exception:
