...
from core.data import HELP_TEXT, INFO_PRO, AddProState, AddTitleChanel, system_message, limit_channel, ai_privilege
import logging
import copy
...
async def chat_shared_handler(message: Message, state: FSMContext):
    user_id = get_simple_user_id(message=message)
    if not user_id:
        return
    answer = await is_user_registered(user_id)
