...
from core.data import model_ai_channel
...
def get_user_channels(user_data):
    channels = user_data.get("dictionary_channel_id") or []
...
async def process_channel_ai(user_id: int, client: AsyncOpenAI, text: str):
    async with get_user_lock(user_id):
        user_dialogue = [{"role": "system", "content": "В твои обязоности входит просмотр сообщений из канала и предоставление краткого содержания, умести содежимое пересказа в 4000 символов, больше символов нельзя. Если в этом канале кого-то отмечали то напиши когои сообщение. Форматируй ответы только в формате Telegram Markddown v1. Не используй заголовки с решётками двойные звездочки и формулы LaTeX($)."}]
        user_dialogue.append({"role": "user", "content": text})

        response = await client.chat.completions.create(
            model=model_ai_channel,
...
async def process_ai(client: AsyncOpenAI, user_id, user_message, name):
    async with get_user_lock(user_id):
        ...
        while len(user_dialogue) > 20:
            user_dialogue.pop(1)
            user_dialogue.pop(1)
        await add_data(user_id, user_dialogue=user_dialogue, total_tokens=completion_tokens)
        return assistant_reply
